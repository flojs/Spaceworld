...
int obj_parse_mtl_file(const char *pathname, const char *filename, list *material_list)
{
	int line_number = 0;
	char *current_token;
    ...
    char pathfile[OBJ_FILENAME_LENGTH];
    strncpy(pathfile, pathname, OBJ_FILENAME_LENGTH);
    strncat(pathfile, filename, OBJ_FILENAME_LENGTH);
    
	// open scene
	mtl_file_stream = fopen( pathfile, "r");
	...
	while( fgets(current_line, OBJ_LINE_SIZE, mtl_file_stream) )
	{
		...
		// illumination type (ignored)
		else if( strequal(current_token, "illum") && material_open)
		{
		}
		// texture map
		else if( ( strequal(current_token, "map_Ka") ||
                   strequal(current_token, "map_Kd") ) && material_open)
		{
            strncpy(current_mtl->texture_filename,
                    pathname, OBJ_FILENAME_LENGTH);
            strncat(current_mtl->texture_filename,
                    strtok(NULL, " \t\r\n"), OBJ_FILENAME_LENGTH);
		}
		else
		{
...
int obj_parse_obj_file(obj_growable_scene_data *growable_data, const char *filename)
{
    ...
    // Extract pathname, if there is one
    char pathname[OBJ_FILENAME_LENGTH];
	...
	while( fgets(current_line, OBJ_LINE_SIZE, obj_file_stream) )
	{
		...
		else if( strequal(current_token, "mtllib") ) // mtllib
		{
			...
			obj_parse_mtl_file(pathname, growable_data->material_filename,  &growable_data->material_list);
			continue;
		}
